...
bool snakeController::update() {
  ...
  if (state_ == MOVING || state_ == START || state_ == GAMEOVER ||
      state_ == PAUSE) {
    sig_ = GET_USER_INPUT;
  }
  return break_flag;
...
void snakeController::QTsig(int sig) { sig_ = sig; }

snakeController::snakeController(snakeModel *m)
    : model(m),
      state_(START),
      sig_(0),
      start_time_(clock()),
...
const snakeModel::Game_Stats &snakeController::getModel() {
  return model
      ->getGameStats();  // Получаем ссылку на game_stats_ через метод модели
}

bool snakeController::start_state() const { return state_ == START; }

bool snakeController::gameover_state() const { return gameover_; }

snakeController::snake_state snakeController::getState() { return state_; }
