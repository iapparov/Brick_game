...
void snakeConsoleView::render(snakeModel::Game_Stats stats) {
  ...
  for (size_t i = 0; i < stats.getApple().size(); i++) {
    ...
    draw_block(stats.getApple().at(i).second, stats.getApple().at(i).first + 1,
               2);
  }
  print_stats(stats.getLevel(), stats.getScore(), stats.getHighscore());
}

void snakeConsoleView::print_rectangle(int top_y, int bottom_y, int left_x,
                                       int right_x) {
...
int snakeConsoleView::performChoice() {
  ...
  if (char_Choice == '1') Choice = TETRIS;
  if (char_Choice == '2') Choice = SNAKE;
  if (char_Choice == ESCAPE) Choice = TERMINATE;
  return Choice;
}
...
void snakeConsoleView::print_gameover() {
  MVPRINTW(BOARD_Y / 2 - 5, (BOARD_X * 2 - GM_MESSAGE_LEN) / 2 + 1, GM_MESSAGE);
}
