...
void GameWidget::paintEvent(QPaintEvent *event) {
    ...
    for (size_t i = 0; i < stats_.getBody().size(); i++) {
        ...
        painter.drawRect(stats_.getBody().at(i).first * 20, stats_.getBody().at(i).second * 20, 20, 20);
    }

    // Отрисовка яблока
    for (size_t i = 0; i < stats_.getApple().size(); i++) {
        painter.setBrush(Qt::red);
        painter.drawRect(stats_.getApple().at(i).first * 20, stats_.getApple().at(i).second * 20, 20, 20);
    }

    // Отрисовка статистики (Level, Score, Highscore)
    QRect statsRect(width() - 250, 55, 200, 150); // Создаем прямоугольник для статистики
    ...
    painter.drawText(statsRect.left() + 10, yPos, "Level: " + QString::number(stats_.getLevel()));
    yPos += 30;  // Добавляем отступ после первого текста

    painter.drawText(statsRect.left() + 10, yPos, "Score: " + QString::number(stats_.getScore()));
    yPos += 30;  // Добавляем отступ после второго текста

    painter.drawText(statsRect.left() + 10, yPos, "Highscore: " + QString::number(stats_.getHighscore()));
}
...
void GameWidget::EnterKey(){
    controller_->QTsig(10);
}
